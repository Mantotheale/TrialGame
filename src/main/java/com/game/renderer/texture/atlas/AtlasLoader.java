...
public class AtlasLoader {
    ...
    public Optional<List<ImageAndMetadata>> loadAtlases() {
        ...
        List<BufferedImage> images = openImages();
    ...
    private List<BufferedImage> openImages() {
        return IOUtils.filesInDirectory(atlasDirectory)
                .stream()
                .filter(p -> !p.equals(checksumPath) && !p.equals(metadataPath))
                .sorted()
                .map(IOUtils::loadImage)
                .toList();
    }
